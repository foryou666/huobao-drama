  ...
  <div class="studio-page">
    <header class="studio-header">
      <div class="studio-header-copy">
        <h1 class="studio-title">视频生成（Grok）</h1>
        <StudioGuideButton
          title="Grok 通道说明"
          text="使用 Grok 视频生成能力；可关联项目并用参考图/提示词创作。具体可用模型和计费规则以页面选项为准。"
        />
      </div>
    ...
    <div ref="feedRef" class="studio-feed">
      ...
      <div v-else-if="!items.length" class="studio-empty card">
        <p>还没有 Grok 视频，在底部输入描述并点击「Grok 生成」</p>
      </div>
    ...
    <div class="studio-composer-wrap">
      <VideoStudioComposer
        ...
        :grok-models="displayModels"
        :fixed-config-id="grokConfigId"
        :fixed-model="selectedModel"
        :credit-cost-flat="selectedCreditCostFlat"
        drama-preference-scope="video-grok"
...
const VIDEO_LEDGER_CACHE_PREFIX = 'studio-video-ledger-grok-v1'
...
const DEFAULT_GROK_MODEL = 'grok-video-3-pro'

const DEFAULT_GROK_MODELS = [
  { id: 'grok-video-1.5-max', label: 'Grok Video 1.5 Max', credit_cost_flat: 20, config_id: null, duration_min: 4, duration_max: 15, duration_default: 15 },
  { id: 'grok-video-1.5-pro', label: 'Grok Video 1.5 Pro', credit_cost_flat: 20, config_id: null, duration_min: 4, duration_max: 10, duration_default: 10 },
  { id: 'grok-video-3-max', label: 'Grok Video 3 Max', credit_cost_flat: 20, config_id: null, duration_min: 4, duration_max: 15, duration_default: 15 },
  { id: 'grok-video-3-pro', label: 'Grok Video 3 Pro', credit_cost_flat: 20, config_id: null, duration_min: 4, duration_max: 10, duration_default: 10 },
]
...
const grokModels = ref([])
const grokApiKeyConfigured = ref(true)
const selectedModel = ref(DEFAULT_GROK_MODEL)
const items = ref([])
const dramas = ref([])
...
const selectedCreditCostFlat = computed(() => {
  ...
  const cost = model?.credit_cost_flat
  return cost != null && Number.isFinite(Number(cost)) ? Number(cost) : 20
})

const displayModels = computed(() =>
  grokModels.value.length ? grokModels.value : DEFAULT_GROK_MODELS,
)

const grokConfigId = computed(() => {
  const model = displayModels.value.find(item => item.id === selectedModel.value)
...
function modelLabel(modelId) {
  return displayModels.value.find(item => item.id === modelId)?.label
    || String(modelId || '').replace(/^grok-video-/, 'Grok ').replace(/-/g, ' ')
}
...
async function loadGrokOptions() {
  try {
    const res = await videoAPI.grokOptions()
    grokModels.value = (res?.models || []).map(item => ({
      ...item,
      credit_cost_flat: item.credit_cost_flat ?? item.credit_cost ?? 20,
    }))
    grokApiKeyConfigured.value = res?.api_key_configured !== false
    if (grokModels.value.length && !grokModels.value.some(item => item.id === selectedModel.value)) {
      const preferred = grokModels.value.find(item => item.id === DEFAULT_GROK_MODEL)
      selectedModel.value = preferred?.id || grokModels.value[0].id
    }
  } catch {
    grokModels.value = []
    grokApiKeyConfigured.value = false
  }
}
...
async function onGenerate(payload) {
  ...
  if (!grokConfigId.value) {
    await loadGrokOptions()
  }
  if (!grokConfigId.value) {
    toast.error('未配置 GeekNow Grok 视频服务，请联系管理员')
    return
  }
  if (!grokApiKeyConfigured.value) {
    toast.error('GeekNow API Key 未配置，请管理员在「设置 → AI 配置」中填写 GeekNow（图片/视频）的 API Key')
    return
  }
  ...
  try {
    const generation = await videoAPI.generate({
      ...payload,
      grok: true,
      config_id: grokConfigId.value,
      model: selectedModel.value,
    })
    toast.success('Grok 视频任务已提交')
...
.studio-grid-skeleton { pointer-events: none; }
.studio-card-skeleton { cursor: default; }
.studio-card-skeleton:hover { transform: none; box-shadow: none; }
...
.studio-skeleton-line { height: 10px; margin-bottom: 8px; border-radius: 4px; }
