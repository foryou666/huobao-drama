  ...
  <div class="studio-page">
    <header class="studio-header">
      <div class="studio-header-copy">
        <h1 class="studio-title">S通道9</h1>
        <StudioGuideButton
          title="S通道9 使用说明"
          :text="`S通道9（星月梦）：参考上限 ${refLimitsHint}。支持 Seedance 2.0 Mini / Fast / Pro，时长 4–15 秒。`"
        />
      </div>
      <div class="studio-header-actions">
        ...
        <select
          v-if="isAdmin && xingyuemengSessions.length > 1"
          v-model="selectedXingyuemengSessionId"
          class="studio-filter-select"
          title="管理员可指定 Session；选「全部」则自动分配"
        >
          <option value="">全部（自动分配）</option>
          <option v-for="s in xingyuemengSessions" :key="s.id" :value="s.id">
            {{ xingyuemengSessionOptionLabel(s) }}
          </option>
        </select>
    ...
    <div class="studio-composer-wrap">
      <VideoStudioComposer
        ...
        :xingyuemeng-models="displayModels"
        :fixed-model="selectedModel"
        :xingyuemeng-resolutions="xingyuemengResolutions"
        :xingyuemeng-default-resolution="xingyuemengDefaultResolution"
        ...
        drama-preference-scope="video-xingyuemeng"
...
import {
  resolveXingyuemengModelPreference,
  setSavedXingyuemengModel,
} from '~/utils/studio-xingyuemeng-model-preference.js'

const VIDEO_LEDGER_CACHE_PREFIX = 'studio-video-ledger-xingyuemeng-v1'

const XINGYUEMENG_MODEL_IDS = [
  'xingyuemeng-video-seedance-2.0-pro',
  'xingyuemeng-video-seedance-2.5',
  'xingyuemeng-video-seedance-2.0-fast',
  'xingyuemeng-video-seedance-2.0-mini',
]

const DEFAULT_XINGYUEMENG_MODEL = 'xingyuemeng-video-seedance-2.0-pro'

const XINGYUEMENG_REF_LIMITS = VIDEO_CHANNEL_REF_LIMITS['/videos/xingyuemeng']

function refLimitsForModel() {
  return XINGYUEMENG_REF_LIMITS
}
...
const DEFAULT_XINGYUEMENG_MODELS = [
  { id: 'xingyuemeng-video-seedance-2.0-pro', label: 'S 2.0 Pro', billing_unit: 'duration_resolution', credit_cost_per_second: null, credit_cost_flat: null, duration_min: 4, duration_max: 15, duration_default: 5, ref_limits: { images: 9, audios: 3, videos: 3 } },
  { id: 'xingyuemeng-video-seedance-2.5', label: 'S 2.5', billing_unit: 'duration_resolution', credit_cost_per_second: null, credit_cost_flat: null, duration_min: 4, duration_max: 15, duration_default: 5, ref_limits: { images: 9, audios: 3, videos: 3 } },
  { id: 'xingyuemeng-video-seedance-2.0-fast', label: 'S 2.0 Fast', billing_unit: 'duration_resolution', credit_cost_per_second: null, credit_cost_flat: null, duration_min: 4, duration_max: 15, duration_default: 5, ref_limits: { images: 9, audios: 3, videos: 3 } },
  { id: 'xingyuemeng-video-seedance-2.0-mini', label: 'S 2.0 Mini', billing_unit: 'duration_resolution', credit_cost_per_second: null, credit_cost_flat: null, duration_min: 4, duration_max: 15, duration_default: 5, ref_limits: { images: 9, audios: 3, videos: 3 } },
]
...
const xingyuemengModels = ref([])
const xingyuemengSessions = ref([])
const selectedXingyuemengSessionId = ref('')
const xingyuemengResolutions = ref(['480p', '720p'])
const xingyuemengDefaultResolution = ref('720p')
const keyConfigured = ref(false)
const keyValid = ref(false)
const selectedModel = ref(DEFAULT_XINGYUEMENG_MODEL)
let modelPreferenceReady = false
const items = ref([])
const dramas = ref([])
...
const displayModels = computed(() =>
  xingyuemengModels.value.length ? xingyuemengModels.value : DEFAULT_XINGYUEMENG_MODELS,
)
...
function xingyuemengSessionOptionLabel(session) {
...
const selectedCreditCostFlat = computed(() => {
  const model = displayModels.value.find(item => item.id === selectedModel.value)
  if (model?.billing_unit === 'duration_resolution') return null
...
function modelLabel(modelId) {
  return displayModels.value.find(item => item.id === modelId)?.label
    || String(modelId || '').replace(/^xingyuemeng-video-/, '').replace(/-/g, ' ')
    || 'S 2.0 Pro'
}
...
async function retryItem(item) {
  ...
  if (model && xingyuemengModels.value.some(m => m.id === model)) {
...
async function reuseDetail() {
  ...
  if (detailItem.value.model && XINGYUEMENG_MODEL_IDS.includes(detailItem.value.model)) {
...
function buildQuery(offset = 0, limit = pagination.value.limit) {
  return {
    ...
    models: XINGYUEMENG_MODEL_IDS.join(','),
  }
}
...
async function loadXingyuemengOptions() {
  try {
    const res = await videoAPI.xingyuemengOptions()
    xingyuemengModels.value = (res?.models || []).map(item => ({
      ...item,
      credit_costs: item.credit_costs || null,
      credit_cost_per_second: item.billing_unit === 'duration_resolution' ? null : (item.credit_cost_per_second ?? null),
      credit_cost_flat: item.billing_unit === 'duration_resolution'
        ? null
        : (item.credit_cost_flat ?? item.credit_cost ?? 0),
    }))
    xingyuemengSessions.value = res?.sessions || []
    keyConfigured.value = !!res?.session_configured || xingyuemengSessions.value.length > 0
    keyValid.value = !!res?.session_valid || xingyuemengSessions.value.some(item => item.valid)
    if (selectedXingyuemengSessionId.value && !xingyuemengSessions.value.some(item => item.id === selectedXingyuemengSessionId.value)) {
      selectedXingyuemengSessionId.value = ''
    }
    if (!isAdmin.value) selectedXingyuemengSessionId.value = ''
    await applyXingyuemengModelPreference(res)
    applyXingyuemengResolutionsForModel(selectedModel.value, res)
  } catch {
    xingyuemengModels.value = []
    xingyuemengSessions.value = []
    keyConfigured.value = false
    keyValid.value = false
    selectedXingyuemengSessionId.value = ''
    modelPreferenceReady = true
  }
}

async function applyXingyuemengModelPreference(res) {
  modelPreferenceReady = false
  const apply = () => {
    const preferred = resolveXingyuemengModelPreference({
      models: xingyuemengModels.value.length ? xingyuemengModels.value : DEFAULT_XINGYUEMENG_MODELS,
      defaultModel: res?.default_model || DEFAULT_XINGYUEMENG_MODEL,
    })
    if (preferred) selectedModel.value = preferred
  }
  apply()
  // 对齐 composer 草稿恢复（其 onMounted 也会写回 fixedModel）
  await nextTick()
  apply()
...
function applyXingyuemengResolutionsForModel(modelId, optionsRes) {
  const model = xingyuemengModels.value.find(item => item.id === modelId)
  const byModel = optionsRes?.resolutions_by_model?.[modelId]
    || model?.resolutions
  const allowed = new Set(['480p', '720p'])
  const fallback = ['480p', '720p']
  const rawList = Array.isArray(byModel) && byModel.length >= 1
    ? byModel.map(item => String(item))
    : (Array.isArray(optionsRes?.resolutions) && optionsRes.resolutions.length >= 1
      ? optionsRes.resolutions.map(item => String(item))
      : fallback)
  const list = [...new Set(rawList.filter(item => allowed.has(item)))]
  xingyuemengResolutions.value = list.length ? list : fallback
  const preferred = String(model?.default_resolution || optionsRes?.default_resolution || '720p')
  xingyuemengDefaultResolution.value = xingyuemengResolutions.value.includes(preferred)
    ? preferred
    : (xingyuemengResolutions.value[0] || '720p')
}

watch(selectedModel, (modelId) => {
  if (!modelId) return
  applyXingyuemengResolutionsForModel(modelId)
  if (modelPreferenceReady) setSavedXingyuemengModel(modelId)
})
...
async function onGenerate(payload) {
  ...
  if (!serviceAvailable.value) await loadXingyuemengOptions()
  if (!serviceAvailable.value) {
    toast.error(keyConfigured.value
      ? 'S通道9 视频服务暂时不可用，请稍后再试或联系管理员'
      : 'S通道9 视频服务尚未启用，请联系管理员')
    return
  }
  ...
  try {
    const generation = await videoAPI.generate({
      ...
      xingyuemeng_session_id: isAdmin.value ? (selectedXingyuemengSessionId.value || undefined) : undefined,
    })
    toast.success('S通道9 任务已提交')
