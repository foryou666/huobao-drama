...
const displayModels = computed(() =>
  xyqModels.value.length ? xyqModels.value : DEFAULT_XYQ_MODELS,
)
...
function xyqKeyOptionLabel(session) {
  const label = session?.label || '未命名'
  const masked = session?.access_key_masked || ''
...
async function retryItem(item) {
  ...
  if (model && xyqModels.value.some(m => m.id === model)) {
...
async function reuseDetail() {
  ...
  if (detailItem.value.model && XYQ_MODEL_IDS.includes(detailItem.value.model)) {
...
async function loadXyqOptions() {
  try {
    const res = await videoAPI.xyqOptions()
    xyqModels.value = (res?.models || []).map(item => ({
      ...item,
...
async function onGenerate(payload) {
  ...
  try {
    const generation = await videoAPI.generate({
      ...
      xyq_session_id: isAdmin.value ? (selectedXyqKeyId.value || undefined) : undefined,
    })
    toast.success('S通道5 任务已提交')
