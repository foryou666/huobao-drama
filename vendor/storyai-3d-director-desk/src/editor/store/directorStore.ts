...
export const useDirectorStore = create<DirectorStore>((set, get) => {
  ...
  return {
    ...
    importHostBlockingLayout: (payload) => {
      const normalized = normalizeHostBlockingLayoutPayload(payload);
      ...
      commitMutation((state) => {
        if (!shouldApplyHostBlockingLayout(state.project.objects, normalized.force)) {
          return state;
        }

        applied = true;
        const nonCharacters = state.project.objects.filter((item) => item.kind !== "character");
        const nextCharacters = normalized.characters.map((entry) => buildHostBlockingCharacterObject(entry));
        const firstCharacterId = nextCharacters[0]?.id ?? null;
        ...
        return {
          ...
          project: {
            ...state.project,
            objects: [...nonCharacters, ...nextCharacters],
          },
        };
      });
