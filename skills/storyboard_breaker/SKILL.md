...
每个镜头聚焦**一段完整叙事节拍**（可含镜内多次景别/运镜变化），描述详尽具体。每个镜头时长 **10-15 秒**；若项目使用 **Seedance 2.0**，优先 **12-15 秒/镜**（一条 storyboard = 一次视频生成，最长 15 秒）。

> **导演风格优先**：若已加载「红果导演」等 director-style，其中关于时长与 `video_prompt` 的规则**覆盖**本 SKILL 的默认值。
...
13. **视频提示词**：`video_prompt`，**在同一镜内**按 3 秒分段描述时间轴（必填）；不要把每个 3 秒拆成独立 storyboard
